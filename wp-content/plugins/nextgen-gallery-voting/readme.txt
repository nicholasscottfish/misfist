...
**For image voting:**
For image voting, add the following tag: `<?php echo nggv_imageVoteForm($image->pid); ?>`, to the gallery display template.

**For gallery voting:**
For gallery voting, add the following tag: `<?php echo nggv_galleryVoteForm($displayed_gallery, $gallery); ?>`, to the gallery display template.
...
NGG v2.x templating system can be confusing for some users. Please see [this page](http://shauno.co.za/nextgen-gallery-v2-display-template-locations/) for more info on which templates are used.

Please be aware, for **image voting**, the form needs to be placed inside the `foreach()` loop in those templates, as that loop is outputting each image.
...
= 2.7 =
* Added code to fix NGG Publuc Uploader bug, allowing it to be used with NGG Voting.
* Fixed gallery voting when using NGG v2.x.
...
= 2.7 =
Fixed gallery voting on NGG v2.x. Also added code to make NGG Voting work with NGG Public Uploader
