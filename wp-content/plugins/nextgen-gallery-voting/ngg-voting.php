...
class nggVoting {
	...
	function __construct() {
		...
		/* deprecated in NGG v2, manually call func in gallery template
	...
	// Front End Functions {
		function wpInit() {
			...
			}else if((isset($_GET['nggv_gid']) && $_GET['nggv_gid']) || isset($_POST['nggv']['vote_gid_id']) && $_POST['nggv']['vote_gid_id']) {
				if(isset($_GET['nggv_gid'])) {
					$options = $this->getVotingOptions($_GET['nggv_gid']);
				}else if(isset($_POST['nggv']['vote_gid_id'])) {
					$options = $this->getVotingOptions($_POST['nggv']['vote_gid_id']);
				}
				$voteFuncs = $this->types[$options->voting_type]['galleryCallback'];
				$votedOrErr = @call_user_func_array(array($voteFuncs['class'], $voteFuncs['catch']), array($this, $options));
				
				//store in class var, so we have it for this page execution
				$this->initCatchVote[$options->gid] = array(
					'gid'=>$options->gid,
					'result'=>$votedOrErr,
					'dateadded'=>date('Y-m-d H:i:s', time())
					);
				
				do_action('nggv_catch_gallery_vote', $options->gid, $this->initCatchVote);
				
				if($_GET['ajaxify']) { //catch the ajax vote
					$form = $this->getGalleryVoteMarkup($options);
		...
		function getGalleryVoteMarkup($options) {
			$gid = $options->gid;
			$voteFuncs = $this->types[$options->voting_type]['galleryCallback'];
			
			//is there a vote happing for this gid right now?
			$votedOrErr = isset($this->initCatchVote[$gid]['result']) ? $this->initCatchVote[$gid]['result'] : '';
		...
		function galleryVoteForm($gid) {
			...
			if(isset($options->enable) && $options->enable) {
				...
				do_action('nggv_gallery_vote_bottom', $options);				
			}
...
function nggv_galleryVoteForm($displayed_gallery, $gallery) {
	global $officalNggVoting;
	...
	if($gid) {
		return $officalNggVoting->galleryVoteForm($gid);
	}
}
//}
?>