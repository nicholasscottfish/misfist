<?php
/**
 * Shows all Blog Networks.
 */
class Bcat_WidgetCategories extends WP_Widget {

	function Bcat_WidgetCategories () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Shows a list of all networks.', SITE_CATEGORIES_I18N_DOMAIN));
		parent::WP_Widget(__CLASS__, __('Networks', SITE_CATEGORIES_I18N_DOMAIN), $widget_ops);
	}
	...
	function form($instance) {
		...
		$defaults = array( 
			...
			'icon_size'				=>	32,
			'show_more_link'		=>	1,
			'landing_link_label'	=>	__('more networks', SITE_CATEGORIES_I18N_DOMAIN)
...
<?php */ ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'per_page' ); ?>"><?php 
				_e('Number of Networks to show:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'ordering' ); ?>" 
				...
				<option value="id" <?php if ($instance['ordering'] == "id") { echo ' selected="selected" '; }?>><?php 
					_e('Network ID', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
			</select>
		...
		<p>
			<label for="<?php echo $this->get_field_id('hide_empty') ?>"><?php _e('Hide Empty Networks:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_counts') ?>"><?php _e('Show Network Count:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('icon_show') ?>"><?php _e('Show Network icons:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_more_link') ?>"><?php _e('Landing Page link below networks list:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			...
			if (($categories) && (count($categories))) {
				...
				if (intval($instance['show_more_link'])) {
					...
					if (isset($instance['landing_link_label']))
						...
						$data['landing']['link_label'] = __('More Networks', SITE_CATEGORIES_I18N_DOMAIN);								
				}			
			}		
