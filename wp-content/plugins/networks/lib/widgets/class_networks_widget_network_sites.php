...
class Bcat_WidgetCategorySites extends WP_Widget {
	...
	function Bcat_WidgetCategorySites () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Shows a list of all sites that belong to selected network.', SITE_CATEGORIES_I18N_DOMAIN));
		parent::WP_Widget(__CLASS__, __('Sites From a Single Network', SITE_CATEGORIES_I18N_DOMAIN), $widget_ops);
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Network:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<?php
		...
		<p>
			<label for="<?php echo $this->get_field_id('include_children') ?>"><?php _e('Include Child (Nested) Networks:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
