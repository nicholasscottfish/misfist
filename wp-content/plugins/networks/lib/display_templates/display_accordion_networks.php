...
//	echo "data<pre>"; print_r($data); echo "</pre>";
	...
	if ((isset($data['categories'])) && (count($data['categories']))) {
		...
		$content .= '<div class="site-categories-accordion">';
		...
		foreach ($data['categories'] as $category) { 
			...
			if ( ($args['icon_show'] == true) && (isset($category->icon_image_src)) && (strlen($category->icon_image_src)) ) {
				...
				$content .= '<div><img class="site-category-icon" alt="'. $category->name .'" src="'. $image_src .'" /></div>';
			} 
			
			$content .= '<div><span class="site-category-title">'. $category->name .'</span>';
			...
			if ((isset($category->children)) && (count($category->children))) {
				...
				foreach( $category->children as $category_child) {
					...
					$content .=	'<li class="site-categories-child">';

					if ( ($args['icon_show_children'] == true) && (isset($category_child->icon_image_src))) {
						$content .= '<img class="site-category-icon" alt="'. $category_child->name .'" src="'. $category_child->icon_image_src .'" />';
					} 
