...
function haku_get_tweets( $user, $number = 3, $haku_tweets = array() ) {
	...
	$tweets_feed = haku_get_contents( 'http://api.twitter.com/1/statuses/user_timeline.json?include_rts=1&screen_name=' . $user . '&count=' . $number . '&callback=?' );
	...
	if ( ! empty( $tweets_feed ) && ! $tweets_feed["error"] ) {
		foreach ( $tweets_feed as $tweet ) {
			$haku_tweets[] = array(
				...
				'date' => haku_nice_date( strtotime( $tweet['created_at'] ), true )
			);
		}
	}
	return $haku_tweets;
