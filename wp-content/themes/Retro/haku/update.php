...
function haku_get_latest_theme_version() {
	...
	if ( ! $last || ( ( time() - $last ) > 86400 ) ) {
		...
		$cache = haku_get_contents( get_haku_var('theme_xml') );
...
function haku_update_notifier_menu() {
	...
	if ( version_compare( $xml->latest, get_theme_version(), '>' ) ) {
		add_dashboard_page( get_haku_var('theme_name') . ' Theme Updates', get_haku_var('theme_name') . '<span class="update-plugins count-1"><span class="update-count">' . $xml->latest . '</span></span>', 'switch_themes', get_haku_var('theme'), 'haku_theme_update_notify');
	}
	
}

add_action('admin_menu', 'haku_update_notifier_menu');  
...
add_action( 'admin_bar_menu', 'haku_update_notifier_bar_menu', 1000 );
...
function haku_theme_update_notify() {
	...
	<div class="wrap">
	    ...
	    <div class="updated"><p><strong><?php printf( __('A new version of %s has just become available!', 'haku'), get_haku_var('theme_name') ); ?></strong> <?php printf( __('Head over %s, login into your account, switch to the "Downloads" section and re-download %s, just like you did the first time.', 'haku'), '<a href="http://themeforest.net" target="_blank">ThemeForest.net</a>', get_haku_var('theme_name') ); ?></p></div>
		...
		<div style="float: left; width: 500px;">
			...
			<p><?php printf( __('Before updating, I recommend you to perform a quick backup of your current version of %s (%s). There&rsquo;s no risk that something might corrupt your database, but you might lose any eventual change you made on the theme itself.', 'haku'), get_haku_var('theme_name'), get_theme_version() ); ?></p>
			...
			<p><?php echo sprintf( __('If you still need help, don&rsquo;t hesitate to contact me through my ThemeForest.net %s Contact Form.', 'haku'), '<a href="http://themeforest.net/user/pacovitiello1?ref=pacovitiello1">' . __('Profile Page', 'haku') . '</a>' ); ?></p>
